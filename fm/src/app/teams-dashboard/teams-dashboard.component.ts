...
export class TeamsDashboardComponent {
  ...
  private setActiveTabFromUrl(url: string) {
    if (url.includes('department')) this.activeNavItem = 'department';
    else if (url.includes('team-space')) this.activeNavItem = 'team-space';
    else if (url.includes('projects')) this.activeNavItem = 'projects';
    else if (url.includes('team-list')) this.activeNavItem = 'team-list';
  }
